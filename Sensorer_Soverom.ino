...
void termostat1(){
  ...
  circusESP32.write(key_temp,tempVerdiResultat1,token1); // Reporter om Termostaten har slått på varme til COT.
  ...
  if (tempVerdiResultat1 > 26){
    for (servopos1 = 0; servopos1 <= 45; servopos1 += 1) { // goes from 0 degrees to 45 degrees
      myservo.write(servopos1);              // tell servo to go to position in variable 'pos'                    
    }
  }
  if (tempVerdiResultat1 < 26){
    for (servopos1 = 45; servopos1 >= 0; servopos1 -= 1) { // goes from 45 degrees to 0 degrees
      myservo.write(servopos1);              // tell servo to go to position in variable 'pos'
    }
  }
...
// ------------------------------------------------------- //
void konsolldimmer (){
  float dimverdi = circusESP32.read(key_Dimmer,token1); // Leser av hvilken verdi den får fra CoT
...
void Brytere (){
  buttonState = circusESP32.read(key_Lys,token1); // Leser av hvilken verdi den får fra CoT
  lysVerdiResultat1 = lysSnitt1(); // Setter en variabel til lysSnitt funksjonen
  if (buttonState == 1){ // Setter lyset i dimmemodus
    konsolldimmer ();
  }
