...
char key_temp[] = "3916"; // Knyttet til Token 2
char key_LED_Varme[] = "1059"; // Knyttet til Token 2  
char key_setTemp[] = "5490"; // Knyttet til Token 2
char key_dimverdi[] = "8827"; // Knyttet til Token 3
char key_nattsenk[] = "30493"; // Knyttet til Token 1
char key_dusj[] = "19570"; // Knyttet til Token 3
char key_vaskmaskin[] = "31572"; // Knyttet til Token 3
char token1[] = "eyJhbGciOiJIUzI1NiJ9.eyJqdGkiOiI1MTkwIn0.-xszm_AFkXDZTnevdjFy9Ivvozdp02EwBSulSWwiOHg"; // Den personlige token som linkes opp til signalene dine
...
void loop(){
  Serial.println("------ CoT - ESP32 - BAD ----"); // Printer startlinje i voidloop
  konsolldimmer(); // Kjører lysstyrings funskjonen 
  termostat(); // Kjører termostatfunksjonen
  dusj(); // Kjører dusjfunksjon
  vaskemaskin(); // Kjører vaskemakin funksjon 
  delay(100);
} 


// ----------------------------------------------- //
//       Funksjon for dusj og vaskemaskin.        //
// --------------------------------------------- //

void dusj(){
  button1.loop();
  if (button1.isPressed()){
    Serial.println("DUSJ knapp trykket");
    buttonState1 = !buttonState1;
    if (buttonState1 = HIGH){
      Serial.println("Dusj er PÅ");
      circusESP32.write(key_dusj,buttonState1,token3);
    }
    else {
      circusESP32.write(key_dusj,buttonState1,token3);
    }
...
void vaskemaskin(){
  ...
  if (button2.isPressed()){
    Serial.println("Vaskemaskin knapp trykket");
    buttonState1 = !buttonState1;
    if (buttonState1 = HIGH){
      Serial.println("Vaskemaskin er PÅ");
      circusESP32.write(key_vaskmaskin,buttonState2,token3);
    }
    else {
      circusESP32.write(key_vaskmaskin,buttonState2,token3);
    }
...
void konsolldimmer (){
  ...
  dimverdi = map(dimverdi, 0, 100, 0, 255); // Får inn en verdi mellom 0 og 100 fra CoT, gjør dette om til 0 --> 255
